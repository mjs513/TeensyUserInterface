...
sliderM powerlevel("powerlevel", sliderM::SLIDER_MODE::SLIDER_MODE_HORIZONTAL);
sliderM powerlevelv( "powerlevelv", sliderM::SLIDER_MODE::SLIDER_MODE_VERTICAL);
...
MENU_ITEM mainMenu[] = {
  ...
  {MENU_ITEM_TYPE_COMMAND,            "Draw slider",                 commandSetPowerLevel,      NULL},
...
void commandSetPowerLevel(void)
{
  ...
  //
  powerlevel.init(  200,  180, 150,  30, true, true);
  powerlevel.setAxis( 1.0, 255.0, 16, 2);
  powerlevel.setBump( 1.0, true, true, true);
  //powerlevel.setValueFont( &Arial_16, LCD_WHITE, LCD_RED);
  powerlevel.setDisabledColors( LCD_MDGREY, LCD_MDGREY, LCD_BLACK, LCD_BLACK, LCD_BLACK);
  powerlevel.setSliderPrint( 3, 0, false);
  powerlevel.drawSlider();

  powerlevelv.init(  420,  180,  30, 150, true, true);
  powerlevelv.drawSlider();
  ...
  {
    ui.getTouchEvents();
    if(powerlevel.checkSliderT())
        powerlevel.drawSlider();

    if(powerlevelv.checkSliderT())
      powerlevelv.drawSlider();
...
/*
    if (ui.checkForSliderTouched(myPowerSlider))
    {
      ...
      ui.lcdDrawFilledRectangle(150, 160, 200, 140, LCD_BLACK);
      ui.lcdSetCursorXY(240, 180);
      ui.lcdPrintCentered(myPowerSlider.value, 2);
    }
*/
